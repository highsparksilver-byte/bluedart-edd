...
async function trackShiprocket(awb) {
  try {
    ...
    const delivered =
      ...
      td.delivered_date ||
      td.shipment_track?.some(s => s["sr-status-label"] === "DELIVERED");
...
async function persistTracking(awb, data) {
  ...
  if (rows.length === 0) {
    console.warn("⚠️ Unknown AWB ignored:", awb);
    return;
  }
